...
function showBannerAtIndex(index) {
    ...
    if (item && item.backdrop_path && bannerElement) {
        ...
        // Setup banner buttons for the current item
        setupBannerButtons(item);
    }
}

// Function to setup banner buttons (play and more information)
function setupBannerButtons(item) {
    const playButton = document.getElementById('play-button');
    const moreInfoButton = document.getElementById('more-info');

    if (!playButton || !moreInfoButton) return;
    ...
    ['click', 'touchstart'].forEach(eventType => {
        newPlayButton.addEventListener(eventType, function(e) {
            e.preventDefault();
            e.stopPropagation();
            // Navigate to movie_details page with the TV show ID
